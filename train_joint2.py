...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    # load in previous frame gaussians
    if re.search(r'room0/', scene.model_path) is None:
        print(scene.model_path)
        next_path = re.sub(r'(^\D*)(\d)+',lambda x: x.group(1)+str(int(x.group(2))-1), scene.model_path)
        print(next_path)
        checkpoint = next_path + "/chkpnt1000.pth"
        model_params = torch.load(checkpoint)
        gaussians.restore(model_params, opt)

        # shutil.copy(next_path + f"pose/pose_{iter}.npy", model_path + "pose/pose_interpolated.npy")
        # assuming we know both the current and previous poses.
        org_pose_prev = np.load(next_path + f"pose/pose_{iter}.npy") # A B
        org_pose_curr = np.load(scene.model_path + f"pose/pose_{iter}.npy") # C D
        print(org_pose_prev)
        print(org_pose_curr)
        org_pose_prev[1] = org_pose_curr[1] @ np.linalg.inv(org_pose_curr[0]) @ org_pose[1] # E = DC^{-1}B
        print(org_pose)
