...
def save_next_pose(model_path, iter, n_views):

    print(model_path)
    next_path = re.sub(r'(^\D*)(\d)+',lambda x: x.group(1)+str(int(x.group(2))+1), model_path)
    print(next_path)
    # shutil.copy(next_path + f"pose/pose_{iter}.npy", model_path + "pose/pose_interpolated.npy")
    org_pose = np.load(model_path + f"pose/pose_{iter}.npy") # A B
    org_pose2 = np.load(next_path + f"pose/pose_{iter}.npy") # C D
    print(org_pose)
    print(org_pose2)
    org_pose[1] = org_pose2[1] @ np.linalg.inv(org_pose2[0]) @ org_pose[1] # E = DC^{-1}B
    print(org_pose)
    np.save(model_path + "pose/pose_interpolated.npy", org_pose)
...
def render_sets(
    ...
    # Get next pose
    save_next_pose(dataset.model_path, iteration, args.n_views)
